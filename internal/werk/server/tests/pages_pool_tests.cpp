...
TEST(PagesPool, Vuln_DeallocateWithoutPool) {
    auto pool = Pool(3);

    auto p1 = pool->Allocate(0x12334234112);
    ASSERT_TRUE(p1.success);
    ASSERT_EQ(p1.page->vd, 0x12334234112);

    auto p2 = pool->Allocate(0x1235667453432);
    ASSERT_TRUE(p2.success);
    ASSERT_EQ(p2.page->vd, 0x1235667453432);

    auto p3 = pool->Allocate(0xdef1235346);
    ASSERT_TRUE(p3.success);
    ASSERT_EQ(p3.page->vd, 0xdef1235346);

    // 'deallocate' with vuln in vm interpreter
    p2.page->memory[1337] = 0xAB;
    p2.page->vd = kEmptyVmDescriptor;

    auto p4 = pool->Allocate(0xdeadbeefcafebabe);
    ASSERT_TRUE(p4.success);
    ASSERT_EQ(p4.page->vd, 0xdeadbeefcafebabe);
    ASSERT_EQ(reinterpret_cast<void*>(p2.page), reinterpret_cast<void*>(p4.page));
    // memory doesnt zeroed
    ASSERT_EQ(p4.page->memory[1337], 0xAB);
}
